 ...
 * 项目审批
 */
angular.module('cmtdataprojbscinfpage.controller', [])
    .controller('CmtdataprojbscinfpageCtrl', function ($scope, $state, GlobalVariable, $ionicBackdrop, $window, $timeout, $ionicHistory,$ionicPopup,$rootScope,$ionicLoading) {
        ...
        function init(initRet) {
            ...
            $scope.carbonCopy = function(){
                ...
                $scope.cstNum ="";
                if($scope.activityId == 'ACT_4'||$scope.activityId == 'ACT_3'){
                    var prjBizNm = $scope.baseData.bizOptNm;
                    ...
                    if (data) {
                        ...
                        if ($scope.cstNum != null && $scope.cstNum != "") {
                            ...
                            AjaxJsonp(SysServiceData('CPC', 'carbonCopyStart', [jsonToXml(requestData), jsonToXml(sysUserVO)]), url.cpc, function (responseVO) {
                                hideCommon($ionicLoading);
                                if (responseVO && responseVO.status == "200") {
                                    var alertPopup = $ionicPopup.alert({
                                        title: '提示',
                                        template: '抄送成功'
                                    });
